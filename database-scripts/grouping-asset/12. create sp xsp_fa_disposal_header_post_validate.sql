...
BEGIN
    ...
    SELECT @missing_asset = STUFF((
        ...
        FROM dbo.FA_DISPOSAL_DETAIL fsd_parent
        ...
        LEFT JOIN dbo.FA_DISPOSAL_DETAIL fsd_child
            ON fsd_child.FA_DISPOSAL_CODE = fsd_parent.FA_DISPOSAL_CODE
           AND LTRIM(RTRIM(fsd_child.barcode)) = LTRIM(RTRIM(fgad_child.barcode))
        WHERE fsd_parent.FA_DISPOSAL_CODE = @p_code_barcode
