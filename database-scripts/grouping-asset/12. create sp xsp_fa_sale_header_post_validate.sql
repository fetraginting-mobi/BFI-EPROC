...
BEGIN
    ...
    SELECT @missing_asset = STUFF((
        ...
        INNER JOIN dbo.fa_grouping_asset_detail fgad_child
            ON fgad_child.fa_ga_code = fgad_parent.fa_ga_code
		INNER JOIN FA_ASSET fa on fa.ID = fgad_child.FA_ASSET_ID and fa.AST_CODE = fgad_child.CODE_ASSET
    ...
    BEGIN
        RAISERROR (
            'This transaction cannot be processed because the asset is still linked to another asset within the same group. Please remove it from the group first, or add all assets within the group. Missing asset: %s',
            16,
            1,
            @missing_asset
        );
