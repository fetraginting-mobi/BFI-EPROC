CREATE PROCEDURE [dbo].[xsp_fa_disposal_header_post_validate]
(
...
BEGIN
	...
	--drop table #TEMPASSETDISPOSAL
    CREATE TABLE #TEMPASSETDISPOSAL (
        CODE_ASSET      NVARCHAR(36),
        BARCODE         NVARCHAR(50),
        FA_DISPOSAL_CODE    NVARCHAR(28),
    ...
    -- 2. Populate Data Penjualan Aset ke #TEMPASSETDISPOSAL
    INSERT INTO #TEMPASSETDISPOSAL (CODE_ASSET, BARCODE, FA_DISPOSAL_CODE, FA_GA_CODE, IS_PARENT)
    SELECT 
        fsd.CODE_ASSET,
        fsd.BARCODE,
        fsd.FA_DISPOSAL_CODE,
        ISNULL(fgad.FA_GA_CODE, ''),
        ISNULL(fgad.IS_PARENT, 0)
    FROM dbo.FA_DISPOSAL_DETAIL fsd WITH (NOLOCK)
    ...
    WHERE fsd.FA_DISPOSAL_CODE = @p_code_barcode;
	...
	CROSS APPLY (
		...
		WHERE fdd.BARCODE = tas.BARCODE
		  AND fdd.FA_DISPOSAL_CODE	<> @p_code_barcode
    ...
    IF EXISTS (SELECT 1 FROM #TEMPASSETDISPOSAL WHERE FA_GA_CODE <> '')
    BEGIN
        ...
        INNER JOIN (SELECT DISTINCT FA_GA_CODE FROM #TEMPASSETDISPOSAL WHERE FA_GA_CODE <> '') temp 
    ...
    BEGIN
		SELECT @invalid_child_assets = STUFF((
			...
			FROM #TEMPASSETGROUPING tag 
			WHERE tag.BARCODE not in (select barcode from #TEMPASSETDISPOSAL) 
				  AND FA_ASSET_ID <> 0
    ...
    IF EXISTS (
        SELECT 1 
        FROM #TEMPASSETGROUPING tag
		inner join  #TEMPASSETDISPOSAL tas on tag.BARCODE = tas.BARCODE and tag.CODE_ASSET = tas.CODE_ASSET
        WHERE tag.IS_PARENT = 1  AND FA_ASSET_ID <> 0
    )
    BEGIN
        SELECT @missing_asset = STUFF((
            ...
            FROM #TEMPASSETGROUPING tag
            LEFT JOIN #TEMPASSETDISPOSAL tas
	...
	IF EXISTS (
		...
		BEGIN
			SELECT @invalid_child_assets = STUFF((
				...
				FROM #TEMPASSETGROUPING tag
				LEFT JOIN #TEMPASSETDISPOSAL tam 
				...
				WHERE tam.BARCODE IS NULL 
				  ...
				  AND tag.FA_GA_CODE IN (
					  SELECT DISTINCT FA_GA_CODE FROM #TEMPASSETDISPOSAL WHERE FA_GA_CODE <> ''
				  )
				FOR XML PATH(''), TYPE
			).value('.', 'NVARCHAR(MAX)'), 1, 2, '');
